import { TextDisplay, StoryText } from './text-system.js';
import { StoryMode } from './story-mode.js';
import { Player, Bullet, Enemy, Particle } from './game-objects.js';
...
export class AnimalShootingGame {
    ...
    initGSAPAnimations() {
        ...
        if (gameContainer) {
            gsap.fromTo(gameContainer, { scale: 0.8, opacity: 0, rotation: -5 }, { scale: 1, opacity: 1, rotation: 0, duration: 1, ease: "back.out(1.7)" });
        }
        // UI要素のアニメーション
        const uiOverlay = document.querySelector('.ui-overlay');
        if (uiOverlay) {
            gsap.fromTo(uiOverlay, { x: -100, opacity: 0 }, { x: 0, opacity: 1, duration: 0.8, delay: 0.3, ease: "power2.out" });
        }
        const instructions = document.querySelector('.instructions');
        if (instructions) {
            gsap.fromTo(instructions, { x: 100, opacity: 0 }, { x: 0, opacity: 1, duration: 0.8, delay: 0.5, ease: "power2.out" });
        }
    }
    startStory() {
    ...
    shoot() {
        ...
        if (now - this.lastShot > this.shotCooldown * 16) {
            const bullet = new Bullet(this.player.getBounds().x + this.player.getBounds().width / 2 - 2, this.player.getBounds().y);
    ...
    spawnEnemy() {
        ...
        if (this.enemySpawnTimer >= this.config.enemySpawnRate) {
            ...
            this.enemySpawnTimer = 0;
            // 難易度調整（最小値で制限）
            this.config.enemySpawnRate = Math.max(this.config.minEnemySpawnRate, this.config.enemySpawnRate - 0.1);
        }
    }
    updateGameObjects() {
        // プレイヤーの更新
    ...
    checkCollisions() {
            ...
            for (let j = this.enemies.length - 1; j >= 0; j--) {
                if (this.bullets[i].collidesWith(this.enemies[j])) {
                    ...
                    for (let k = 0; k < particleCount; k++) {
                        this.particles.push(new Particle(enemyBounds.x + enemyBounds.width / 2, enemyBounds.y + enemyBounds.height / 2, enemyBounds.color || '#FF6B6B'));
                    }
                    ...
                    // 敵を倒した時のメッセージ
                    this.textDisplay.addMessage("+100点！", enemyBounds.x, enemyBounds.y, 1000, 16, '#F1C40F');
                    // ステージ進行チェック
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            if (this.player.collidesWith(this.enemies[i])) {
                ...
                for (let k = 0; k < particleCount; k++) {
                    this.particles.push(new Particle(this.player.getBounds().x + this.player.getBounds().width / 2, this.player.getBounds().y + this.player.getBounds().height / 2, '#FF6B6B'));
                }
    ...
    draw() {
        ...
        if (!this.storyPaused) {
            ...
            this.renderer.drawStageInfo(this.storyMode.getCurrentStage(), stageName, this.enemiesDefeated, this.stageTarget);
        }
    }
    getStageName() {
    ...
    showGameOver() {
        ...
        if (gameOverElement && finalScoreElement) {
            ...
            // GSAPゲームオーバーアニメーション
            if (typeof gsap !== 'undefined') {
                gsap.fromTo(gameOverElement, { scale: 0, opacity: 0, rotation: 180 }, { scale: 1, opacity: 1, rotation: 0, duration: 1, ease: "back.out(1.7)" });
                // スコアのカウントアップアニメーション
